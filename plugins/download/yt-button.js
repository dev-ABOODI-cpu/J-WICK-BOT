const handler = async (m, { conn, text }) => {
    ...
    // التحقق من وجود نتائج للبحث لتجنب الانهيار (Crash)
    if (!data || data.length === 0) {
        return m.reply("❌ لم يتم العثور على نتائج لهذا البحث. جرب كلمة أخرى.");
    }
    ...
    await conn.sendButton(m.chat, {
        ...
        mentions: [m.sender],
        newsletter: { name: "𝗝 𝗪𝗶𝗰𝗸 ~ 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 🕷️", jid: "120363425546384005@newsletter" },
